...
char archivoFavs[1000]; // Ruta del archivo donde se guardan los favoritos
...
void manejarFavs(char *opcion, char *parametro) {
   if(strcmp(opcion, "crear") == 0 && parametro != NULL) {                       // "ejecuta" la opción crear
       ...
       strcpy(archivoFavs, parametro);
       strcat(archivoFavs, "/archivo.txt");
...
int main(){
 ...
 while(1){
   ...
   // Guardar favoritos
   Fav fav;
   fav.comando = strdup(input);
   fav.id = contadorFavs;
   favoritos[contadorFavs] = fav;
   contadorFavs += 1;
